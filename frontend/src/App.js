...
function App() {
  ...
  async function getSourceCurrencies() {
    try {
      const res = await fetch('http://localhost:4000/target/api');
      const data = await res.json();
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const res = await fetch(`http://localhost:4000/convert/api?amount=${values.amount}&symbol=${values.sourceCurr}&convert=${values.targetCurr}`);
